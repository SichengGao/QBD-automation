...
def header_index_by_names(ws, names):
    """Return 0-based index if any header contains one of names (case-insensitive), otherwise None."""
...
def detect_memo_column(ws, container_re, sample_rows=50):
    """Pick column with most occurrences of container-like strings or 'material'."""
    num_cols = len(ws[1])
    scores = [0] * num_cols
...
def detect_amount_column(ws, sample_rows=50):
    """Pick column with most numeric-like values."""
    num_cols = len(ws[1])
    scores = [0] * num_cols
    max_check = min(sample_rows, ws.max_row - 1)
    for r in range(2, 2 + max_check):
...
def update_excel(file_path):
    ...
    if DEBUG:
        ...
        print(f"[DEBUG] Columns used - vendor:{col_vendor}, code:{col_code}, memo:{col_memo}, class:{col_class}, amt:{col_amt}, desc:{col_desc}, addr:{col_addr}, terms:{col_terms}")
    ...
    # ==============================
    # BUILD BILL LOOKUP (for zero amount inherit)
    # ==============================
    ...
    for raw_row in filtered_rows:
        ...
        # 3️⃣ Container auto-fix
        if not container_pattern.search(memo):
            cls = str(row[col_class] or "").strip()
            if container_pattern.search(cls):
                row[col_memo] = f"{cls}_{memo}"
                memo = row[col_memo]
                memo_lower = memo.lower()

        new_rows.append(row.copy())

        # Material → Service Fees duplicate
        if "material" in memo_lower and "service fees" not in memo_lower:
            ...
            def ensure_len(lst, idx):
                if idx >= len(lst):
                    lst += [None] * (idx - len(lst) + 1)

            ensure_len(service_row, col_code)
            service_row[col_code] = "53000"

            ensure_len(service_row, col_amt)
            row_text_concat = " ".join(str(x or "") for x in service_row)
            container_found = bool(container_pattern.search(row_text_concat))
            if not container_found:
                for cellval in raw_row:
                    if cellval is None:
                        continue
                    if container_pattern.search(str(cellval)):
                        container_found = True
                        break
            service_row[col_amt] = 750.00 if container_found else 100.00

            ensure_len(service_row, col_desc)
            service_row[col_desc] = "Service Charges"
            ensure_len(service_row, col_addr)
            service_row[col_addr] = "Coastmax"
            ensure_len(service_row, col_terms)
            if not service_row[col_terms]:
                service_row[col_terms] = "Due on receipt"

            if DEBUG:
                print(f"[DEBUG] Creating service row for memo='{memo}' -> container_found={container_found} -> amt={service_row[col_amt]}")
    ...
    for row in new_rows:
        ...
        def ensure_len_row(r, idx):
            if idx >= len(r):
                r += [None] * (idx - len(r) + 1)
            return r

        row = ensure_len_row(row, max(col_vendor, col_code, col_memo, col_class, col_amt, col_desc, col_addr, col_terms))
        row[col_vendor] = "Coastmax"

        memo = str(row[col_memo] or "").strip()
        memo_lower = memo.lower()
        matched = False

        # 2️⃣ Zero amount inherit logic
        try:
    ...
    new_path = os.path.join(folder, f"{name}_ready_for_trader{ext}")
    wb.save(new_path)
